...
void ocall_print_file(const char *str, const char *file, int append) {
  ...
  stream.open(destination_dir + "/" + std::string(file),
              append == 0 ? std::ofstream::out : std::ofstream::app);
  ...
  if (!stream) std::cout << "Write failed" << std::endl;
...
int SGX_CDECL main(int argc, char *argv[]) {
  ...
  worker.OnMessage(
      [&worker, &argc, &argv, &global_eid, &validation_code, &validation_msg](std::string msg) {
        ...
        if (dida_command.compare(dida_dsp) == 0) {
          ...
          mrg_command.Parse(&validation_code, &validation_msg);
          printf("Merge Command parsed...");
          if (validation_code == 0) {
            destination_dir = mrg_command.GetResultsDir();
            printf("Validation passed!");
