...
void distPar(const char *uName, const char *dName, const int pNum, std::vector<int> &disArr) {
...
void distTarget(const char *uName, const char *dName, const int pNum) {
    std::ifstream uFile(uName);
...
int main(int argc, char **argv) {
    ...
    const char *uName(argv[argc - 2]);  // name of target, eg. contig file in .fa format
    const char *dName(argv[argc - 1]);  // name of destination folder
